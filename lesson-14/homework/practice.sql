--1 Easy Tasks
select SUBSTRING(name, 1, CHARINDEX(',', name)-1) as f_name,
	SUBSTRING(name, CHARINDEX(',', name)+1, len(name)) as l_name
...
--3
select left(Vals, CHARINDEX('.', Vals) - 1) AS FirstPart,
    SUBSTRING(Vals, CHARINDEX('.', Vals) + 1, LEN(Vals)) AS RemainingPart
FROM Splitter;
...
--Medium Tasks
--1
CREATE TABLE SampleData (input_string VARCHAR(100));

INSERT INTO SampleData VALUES ('rtcfvty34redt');
...
SELECT SUBSTRING('sdgfhsdgfhs@121313131', n, 1) AS char_val
FROM Numbers;
...
--8
SELECT CAST(LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'X') - 1) AS INT) AS leading_number
FROM GetIntegers;

--1 Difficult Tasks
SELECT id,
       CONCAT(
           SUBSTRING(SUBSTRING(Vals, ',', 2), ',', -1),  -- 2nd value
           ',',
           SUBSTRING(Vals, ',', 1),                           -- 1st value
           SUBSTRING(Vals, charindex(',', vals, charindex(',', vals) + 1)) -- rest
       ) AS swapped_val
FROM MultipleVals;
...
SELECT s.Area,
       ...
       s.TotalSales,
       ROUND(
           (s.TotalSales * 100.0 / NULLIF(w.WeekTotal, 0)),
           2
       ) AS PercentageOfWeek
FROM SalesWithTotal s
JOIN WeeklyTotals w
  ON s.FinancialWeek = w.FinancialWeek
 AND s.FinancialYear = w.FinancialYear
ORDER BY s.FinancialYear, s.FinancialWeek, s.DayOfWeek, s.Area;
