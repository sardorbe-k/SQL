...
--4
select 
  manager_id AS Boshqaruvchi_ID,
  SUM(salary) AS Jami_Oylik
from employees
group by manager_id;
...
--8
SELECT 
  COALESCE(id, ssn, passportid, itin) AS Preferred_Contact
FROM person;
...
--3
SELECT CONCAT(string , ', ') AS Concatenated_Values
FROM DMLTable;
...
SELECT 
  ...
  ROUND(
    100.0 * SUM(CASE WHEN hire_date <= CURRENT_DATE - INTERVAL 3 year  THEN 1 ELSE 0 END) / COUNT(*),
    2
...
--6
SELECT JobDescription, SpacemanID AS most_experienced
FROM Personal
WHERE MissionCount = (
...
DECLARE @special VARCHAR(100) = '';

WHILE @i <= @len
BEGIN
    SET @char = SUBSTRING(@str, @i, 1);

    IF @char COLLATE Latin1_General_CS_AS BETWEEN 'A' AND 'Z'
        SET @upper += @char;
    ELSE IF @char COLLATE Latin1_General_CS_AS BETWEEN 'a' AND 'z'
        SET @lower += @char;
...
-- Declare variables
DECLARE @Equation VARCHAR(100), @SQL NVARCHAR(200), @Result FLOAT;

-- Cursor to loop through all equations
...
BEGIN
    ...
    INSERT INTO #Results (Equation, Result)
