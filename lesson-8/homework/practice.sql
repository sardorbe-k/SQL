...
select 
	...
	else 'sufficent' end as stockstatus 
from products

--8
select country, sum(CustomerID) as total_customer from Customers
group by Country
...
select 
	...
	avg(TotalAmount) as avg_orderdate
from Orders
group by OrderDate
order by OrderDate
...
select top 3 CustomerID, sum(TotalAmount) as total  from Invoices
group by CustomerID
order by total DESC
